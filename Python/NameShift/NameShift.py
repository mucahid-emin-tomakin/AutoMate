...
# Target path where folders/files are located
FOLDER_PATH = r"C:\Users\USER\Documents\Projekte"
# Log file for folder and file names
LOG_FILE = r"C:\Users\USER\Downloads\folder_file_list.txt"
# Log file for rename operations
RENAME_LOG_FILE = r"C:\Users\USER\Downloads\rename_log.csv"
# Toggle which steps to execute
RENAME_FOLDERS = True
LOG_FOLDER_FILE_NAMES = True
RENAME_FILES = True
# File extension filter for file rename (only rename files with these extensions)
RENAME_FILE_EXTENSIONS = [".html"]
# Characters to remove from filenames
REMOVE_CHARS = ["(", ")"]
# Encoding for log files
FILE_ENCODING = "utf-8"
# Separator for CSV log
CSV_SEPARATOR = ";"
# Date format: original order in folder name (DD.MM.YYYY or MM.DD.YYYY etc.)
DATE_INPUT_ORDER = ["day", "month", "year"]
# Date format: desired output order
DATE_OUTPUT_ORDER = ["year", "month", "day"]
# Date separator in folder names
DATE_SEPARATOR = "."
# Minimum folder name length to consider as having a date pattern
...
def log_rename(old_name, new_name):
    """Log rename operation to CSV file."""
...
def reorder_date(date_part):
    """Reorder a date string from input order to output order."""
...
def extract_parts(folder_name):
    """
    Extract first part, date part, and last part from folder name.
    Expected format: FirstPart-DD.MM.YYYY-LastPart or FirstPart-DD.MM.YYYY
    """
...
def rename_folders():
    """
    Rename folders by reordering the date within the folder name.
    Only renames if folder name length > MIN_LENGTH_FOR_DATE.
    """
...
def log_folder_file_names():
    """Log all folder and file names recursively to a text file."""
...
def rename_files():
    """Remove specified characters from filenames. Only targets files with configured extensions."""
...
def main():
    """Main function that executes the selected steps."""
