...
# ==================================================
#  LOGGING (alles in downloader.log)
# ==================================================
...
# ==================================================
#  KONFIGURATION (wie in der Batch)
# ==================================================
...
def collect_expected_ids(ytdlp, input_file):
    ...
    if not input_file.exists():
        log.error(f"Listendatei {input_file} nicht gefunden")
        return []
    with open(input_file, 'r', encoding='utf-8') as f:
        urls = [line.strip() for line in f if line.strip()]
    expected_ids = []
    for url in urls:
        log.debug(f"Sammle IDs von: {url}")
        cmd = [ytdlp, "--flat-playlist", "--print", "id", url]
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, check=True, timeout=60)
            ids = [line.strip() for line in result.stdout.splitlines() if line.strip()]
            if not ids:
                # Keine ID gefunden – vielleicht eine einzelne Video-URL?
                # yt-dlp --print id gibt auch bei einzelnen Videos die ID aus.
                log.warning(f"Keine IDs für {url} gefunden.")
            expected_ids.extend(ids)
            log.info(f"  → {len(ids)} IDs extrahiert")
        except subprocess.CalledProcessError as e:
            log.error(f"Fehler beim Extrahieren von {url}: {e}")
        except subprocess.TimeoutExpired:
            log.error(f"Zeitüberschreitung bei {url}")
    return expected_ids
def get_archive_ids(archive_file):
    ...
    # Jede Zeile hat Format "youtube ID" oder ähnlich – wir nehmen den letzten Teil nach Leerzeichen
    ids = []
    for line in lines:
        parts = line.split()
        if parts:
            ids.append(parts[-1])  # die ID ist meist das letzte Wort
    return ids
def check_completeness(expected_ids, archive_ids, output_dir, mode):
    """Prüft, ob alle erwarteten Videos vorhanden sind."""
    mp3_count = len(list(output_dir.glob("*.mp3"))) if mode == "MP3" else len(list(output_dir.glob("*.mp4")))
    ...
    print(f"Tatsächliche {mode}-Dateien im Ordner:   {mp3_count}")
    if missing_ids:
        print(f"\n❌ Fehlende Videos ({len(missing_ids)}):")
        for vid in list(missing_ids)[:10]:  # maximal 10 anzeigen
            print(f"   - {vid}")
        if len(missing_ids) > 10:
            print(f"   ... und {len(missing_ids)-10} weitere. Siehe Logdatei.")
        log.warning(f"Fehlende IDs: {missing_ids}")
    else:
...
# ==================================================
#  DYNAMISCHE 4‑ZEILEN‑ANZEIGE (wie PowerShell)
# ==================================================
...
def main():
    log.info("=== YouTube Downloader mit Vollständigkeitsprüfung gestartet ===")
    ...
    log.info(f"Format-Auswahl: {choice}")
    if choice == "1":
        MODE = "MP3"
        LIST_FILE = LIST_MP3
    ...
    log.info(f"Erwartete IDs: {len(expected_ids)}")
    if not expected_ids:
    ...
    log.info(f"Starte {MODE}-Download mit Befehl: {' '.join(cmd)}")
    try:
        ...
        if process.returncode != 0:
            log.error(f"yt-dlp beendet mit Code {process.returncode}")
        else:
            log.info("Download erfolgreich beendet")
    except Exception as e:
        log.exception(f"Fehler beim Starten von yt-dlp: {e}")
    finally:
        sys.stdout.write("\033[?25h")
